...
//$("#max-rule").change(function () {
//    lifeMax = $("#max-rule").val();
//});
//
//$("#min-rule").change(function () {
//    lifeMin = $("#min-rule").val();
//});
//
//$("#birth-rule").change(function () {
//    birthRule = $("#birth-rule").val();
//});
...
function redNeighbors(x, y) {
    var number = 0;
    number = (cells[x + 1][y].state === 2) ? number + 1 : number;
    number = (cells[x + 1][y + 1].state === 2) ? number + 1 : number;
    number = (cells[x + 1][y - 1].state === 2) ? number + 1 : number;
    number = (cells[x - 1][y].state === 2) ? number + 1 : number;
    number = (cells[x - 1][y + 1].state === 2) ? number + 1 : number;
    number = (cells[x - 1][y - 1].state === 2) ? number + 1 : number;
    number = (cells[x][y + 1].state === 2) ? number + 1 : number;
    number = (cells[x][y - 1].state === 2) ? number + 1 : number;
...
//    if (neighbors(x, y) === birthRule) {
//        cells[x][y].buffState = 1;
//    } else if (neighbors(x, y) < lifeMin || neighbors(x, y) > lifeMax) {
...
function lifeDeath(x, y) {
    ...
    var numRedNeighbors = redNeighbors(x, y);
    var cont = false;
    var red = false;
    ...
    if (cells[x][y].state === 2) {
        for (var j = 0; j < blackArray.length; j++) {
            if (numRedNeighbors === blackArray[j]) {
                red = true;
            }
        }
        if (red === false) {
        ...
        for (var i = 0; i < redArray.length; i++) {
            if (numNeighbors === redArray[i]) {
                cells[x][y].buffState = 2;
                break;
            }
        }
    }
